...
def main():
    try:
        ...
        job_queue.run_repeating(send_reminder, interval=config["reminder_interval"], first=10)
        leaderboard_time = datetime.strptime(config["leaderboard_time"], "%H:%M").time()
        job_queue.run_daily(update_leaderboard, time=leaderboard_time, days=(0, 1, 2, 3, 4, 5, 6))
